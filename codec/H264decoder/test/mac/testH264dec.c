...
int main(int argc, const char * argv[])
{
    ...
    unsigned long	GVE_H264Dec_Handle = 0;
    ...
    memset(RtpCountBuffer, 0, fpRtpCount_Len *sizeof(char));
    if(RtpCountBuffer == NULL)
    {
        exit(1);
    }
    RtpSizeBuffer = (short *)malloc(fpRtpSize_Len *sizeof(short));
    memset(RtpSizeBuffer, 0, fpRtpSize_Len *sizeof(char));
    ...
    memset(OperatePar.InBuf, 0, INBUF_SIZE *sizeof(char));
    if(OperatePar.InBuf == NULL)
    {
        exit(1);
    }
    OutBuffer = malloc(width * height *3 /2);
    memset(OutBuffer, 0, width * height *3 /2);
    if(OutBuffer == NULL)
    {
        exit(1);
    }
