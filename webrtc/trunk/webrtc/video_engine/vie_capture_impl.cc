...
//	 LeaveCriticalSection(&sec1);
	...
	ret=  shared_data_->input_manager()->NumberOfCaptureDevices();
...
int ViECaptureImpl::SetCaptureFlashlight(const int capture_id, bool on)
{
  WEBRTC_TRACE(kTraceStateInfo, kTraceVideo, ViEId(shared_data_->instance_id()),
               "%s(flashlight: %d)", __FUNCTION__, on);
...
//  __android_log_print(ANDROID_LOG_DEBUG, "WEBRTC", "ViECaptureImpl::SetCaptureFlashlight");
  if (vie_capture->SetCaptureFlashlight(on) != 0) {
    shared_data_->SetLastError(kViECaptureDeviceUnknownError);
    return -1;
  }
  return 0;
