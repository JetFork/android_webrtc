...
int ViEInputManager::CreateVirtualDataPlayer(char* buffer,
									  unsigned int length,
									  const VideoCaptureCapability& frameinfo, int fileID){


	MapItem* map_item = vie_frame_provider_map_.Find(fileID);
	if (!map_item) 
	{
	return 0;
		}
	 ViEVirtualFilePlayer* vie_virtual_file_player =  static_cast<ViEVirtualFilePlayer*>(map_item->GetItem());
	 vie_virtual_file_player->buffer = new char[length];
	memset(vie_virtual_file_player->buffer,0,length);
	 memcpy(vie_virtual_file_player->buffer,buffer,length);
	vie_virtual_file_player->length = length;
	 vie_virtual_file_player->video_codec_info_.width_used =frameinfo.width;
	vie_virtual_file_player->video_codec_info_.height_used = frameinfo.height;
	 vie_virtual_file_player->video_codec_info_.codecType = frameinfo.codecType;
	 //vie_virtual_file_player->FileDataDeProsess();

	 delete vie_virtual_file_player->buffer;
	 return 0;
}
int ViEInputManager::CreateDataPlayer(char* buffer,
									  unsigned int length,
									  const VideoCaptureCapability& frameinfo,int fileID){
 
  MapItem* map_item = vie_frame_provider_map_.Last(); 
  if (!map_item) {
	  return -1;
  }
  ViEFilePlayer* vie_file_player = static_cast<ViEFilePlayer*>(map_item->GetItem());
  vie_file_player->buffer = new char[length];
  memset(vie_file_player->buffer,0,length);
  memcpy(vie_file_player->buffer,buffer,length);
  vie_file_player->length = length;
  vie_file_player->FileDataDeProsess();

  delete vie_file_player->buffer;
  return 0;
}
...
int ViEInputManager::DestoryVirtualDataProvide( int& file_id)
{
	// We need exclusive access to the object to delete it.
	// Take this write lock first since the read lock is taken before map_cs_.
	ViEManagerWriteScoped wl(this);
// ViEVirtualFilePlayer* vie_file_player = NULL;
//	vie_file_player = ViEVirtualFilePlayer(file_id);
	MapItem* map_item = vie_frame_provider_map_.Find(file_id);
	if (!map_item) {
		return 0;
	}
	ViEVirtualFilePlayer* vie_virtual_file_player = 
		static_cast<ViEVirtualFilePlayer*>(map_item->GetItem());

	if (!vie_virtual_file_player)
	{
		WEBRTC_TRACE(webrtc::kTraceError, webrtc::kTraceVideo, ViEId(engine_id_),
	...
	int num_callbacks = vie_virtual_file_player->NumberOfRegisteredFrameCallbacks();
	if (num_callbacks > 0) {
		WEBRTC_TRACE(webrtc::kTraceWarning, webrtc::kTraceVideo,
	...
	// object might cause deletions of renderers so we prefer to not have a lock
	// at that time.
delete vie_virtual_file_player;
	return 0;
}
int ViEInputManager::CreateVirtualDataProvide(const char* file_nameUTF8,
									  const bool loop,
									  const webrtc::FileFormats file_format,
									 /* VoiceEngine* voe_ptr, */int& file_id) {
										  WEBRTC_TRACE(webrtc::kTraceInfo, webrtc::kTraceVideo, ViEId(engine_id_),
										  ...
										  if (GetFreeFileId(&new_file_id) == false) {
											  ...
											  return kViEFileMaxNoOfFilesOpened;
                                                                                 ...
                                                                                 ViEVirtualFilePlayer* vie_file_player = NULL;//ViEVirtualFilePlayer::CreateViEVirtualFilePlayer(
//											  new_file_id, engine_id_, file_nameUTF8, loop, file_format/*, voe_ptr*/);
										  if (!vie_file_player) {
											  ReturnFileId(new_file_id);
											  WEBRTC_TRACE(webrtc::kTraceError, webrtc::kTraceVideo, ViEId(engine_id_),
