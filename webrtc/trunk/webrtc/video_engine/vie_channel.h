...
//typedef void (*DecodedDataCallback)(void * buffer,int width, int height);
#include <list>
...
class ViEChannel
 ...
 public:
  ...
  int RegisterDecodedDataCallback(DecodedDataCallback callback);

  int DeregisterDecodedDataCallback();
  ...
  //kmm add
  /**
  * @brief 修改WebRTC原有代码，使图像增强不仅作用于远程图像，也作用于本地图像。
  * @par 需求：
  GVE-SRS-153
  *@par 详细描述：
  此函数基于WebRTC原有代码修改，仅删除色彩增强处理代码，其他代码不变。\n
  原有WebRTC仅对远程图像进行了色彩增强，因此在通道中进行了色彩增强处理，而现有算法将色彩增强处理置于渲染之前，无关本地或远程。
  */
  ...
  WebRtc_Word32 ReceivedDecodedYUVData(const I420VideoFrame *_frame);
 ...
 private:
  ...
  DecodedDataCallback m_DecodedDataCallback;
