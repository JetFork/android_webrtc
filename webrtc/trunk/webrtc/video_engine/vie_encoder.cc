...
ViEEncoder::ViEEncoder(WebRtc_Word32 engine_id,
  ...
  WEBRTC_TRACE(webrtc::kTraceMemory, webrtc::kTraceVideo,
               ...
               this);
__android_log_print(ANDROID_LOG_ERROR, "kmm","%d vieencoder create engine_id: %d, channel_id:%d", __LINE__, engine_id, channel_id);
...
WebRtc_Word32 ViEEncoder::SetEncoder(const webrtc::VideoCodec& video_codec) {
  WEBRTC_TRACE(webrtc::kTraceInfo, webrtc::kTraceVideo,
               ...
               video_codec.codecType, video_codec.width_used, video_codec.height_used);

  __android_log_print(ANDROID_LOG_ERROR, "yyf","ViEEncoder::SetEncoder width = %d, height =%d\n", video_codec.width_used, video_codec.height_used);

  // Setting target width and height for VPM.
  if (vpm_.SetTargetResolution(video_codec.width_used, video_codec.height_used,
...
void ViEEncoder::DeliverFrame(int id,
               ...
               video_frame->timestamp());
  {
    CriticalSectionScoped cs(data_cs_.get());
      ...
      VideoCodec enc;
      GetEncoder(&enc);
      if (video_frame->width() * enc.width_used == enc.height_used * video_frame->height()) {
  __android_log_print(ANDROID_LOG_ERROR, "yyf","ViEEncoder::encode change encw = %d, ench = %d, width = %d, height =%d\n", enc.width_used, enc.height_used, video_frame->width(), video_frame->height() );
          unsigned short temp = enc.width_used;
          enc.width_used = enc.height_used;
          enc.height_used = temp;
          SetEncoder(enc);
          WEBRTC_TRACE(webrtc::kTraceStateInfo,
               webrtc::kTraceVideo,
               ViEId(engine_id_, channel_id_),
               "%s: change SetEncoder %dx%d", __FUNCTION__,
               enc.width_used, enc.height_used);
      }
...
int ViEEncoder::GetPreferedFrameSettings(int* width,
  ...
  *width = video_codec.width_used;
  *height = video_codec.height_used;
