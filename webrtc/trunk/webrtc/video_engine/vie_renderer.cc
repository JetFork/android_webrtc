...
void ViERenderer::Snapshot(const char *filename)
{
	photo_ = new JpegEncoder;
	photo_->SetFileName(filename);
	is_snapshot_ = true;
}

void ViERenderer::ConfigureRenderMode(const int scaledmode)
{
	if(render_callback_)
		render_callback_->ConfigureRenderMode(scaledmode);
...
{
   // WEBRTC_TRACE(webrtc::kTraceStateInfo, webrtc::kTraceVideo, 0, "DeliverFrame render_id:%d", render_id_);
   if (is_snapshot_)
	{
		photo_->Encode(*video_frame);
		is_snapshot_ = false;
		delete photo_;
		photo_ = NULL;
	}
//   __android_log_print(ANDROID_LOG_ERROR, "yyf","ViERenderer::DeliverFrame, frame_deliver before");
	if (frame_Deliver_ != NULL) {
//		__android_log_print(ANDROID_LOG_ERROR, "yyf","ViERenderer::DeliverFrame, frame_deliver in");
        frame_Deliver_->FrameDeliver(id);
    }
    render_callback_->RenderFrame(render_id_, *video_frame);
}

void ViERenderer::SetRenderFrameDeliver(RenderFrameDeliver* frameDeliver) {
//	__android_log_print(ANDROID_LOG_ERROR, "yyf","ViERenderer::SetRenderFrameDeliver");
    frame_Deliver_ = frameDeliver;
}
//kmm add
int ViERenderer::RenderResolutionProcess(I420VideoFrame* video_frame, int type)
{
	//@Todo
	return 0;
}
