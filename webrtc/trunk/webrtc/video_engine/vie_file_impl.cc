...
int ViEFileImpl::DestoryVirtualPlayFile(
								   int& file_id) 
{
	 ViEInputManagerScoped is(*(shared_data_->input_manager()));
// Destroy the capture device.
return shared_data_->input_manager()->DestoryVirtualDataProvide(file_id);

}
int ViEFileImpl::CreateVirtualPlayFile( int& file_id){
  WEBRTC_TRACE(kTraceApiCall, kTraceVideo, ViEId(shared_data_->instance_id()),
	  "%s", __FUNCTION__);
 ...
 // VoiceEngine* voice = shared_data_->channel_manager()->GetVoiceEngine();
  const WebRtc_Word32 result = shared_data_->input_manager()->CreateVirtualDataProvide(
	  NULL, false, kFileFormatWavFile, /*voice,*/ file_id);
  if (result != 0) {
	  shared_data_->SetLastError(result);
	  return -1;
  }
...
int ViEFileImpl::SetVirtualPlayData(char* buffer,unsigned int length,const Capability& frameInfo,int fileID)
{

	VideoCaptureCapability capabily;
	capabily.height = frameInfo.height;
	capabily.width	= frameInfo.width;
	capabily.maxFPS = frameInfo.maxFPS;
	capabily.rawType = frameInfo.rawType;
...
/*
	std::map<WebRtc_Word32, WebRtc_Word32>::iterator it = m_fileidandchannelid_map.find(fileID);
	if (it == m_fileidandchannelid_map.end()) {
		return -1;
	}
	int videochannelid= it->second;

	ViEChannelManagerScoped cs(*(shared_data_->channel_manager()));
	ViEEncoder* vie_encoder = cs.Encoder(videochannelid);
	if (!vie_encoder) {
	...
	vie_encoder->GetEncoder(&video_codec);
     ...
     if(video_codec.width!= capabily.width||video_codec.height!=capabily.height)
	 {


		 int size_y = capabily.width * capabily.height;
		 int half_width = (capabily.width + 1) / 2;
		 int half_height = (capabily.height + 1) / 2;
		 int size_uv = half_width * half_height;

		 uint8_t* buffer_y = (uint8_t*)buffer;
		 uint8_t* buffer_u = (uint8_t*)buffer_y + size_y;
		 uint8_t* buffer_v =(uint8_t*) buffer_u + size_uv;
		 _captureFrame.CreateFrame(size_y, buffer_y,  size_uv, buffer_u, size_uv, buffer_v,  capabily.width, capabily.height,capabily.width, half_width, half_width);



		 _resampledFrame.CreateEmptyFrame(video_codec.width,video_codec.height,video_codec.width,video_codec.width/2,video_codec.height/2);

		VPMSpatialResampler*     _spatialResampler;
		_spatialResampler = new VPMSimpleSpatialResampler();
		 _spatialResampler->SetTargetFrameSize(video_codec.width,video_codec.height);
		  
			 if (_spatialResampler->ApplyResample(capabily.width, capabily.height)) 
			 {
				 WebRtc_Word32 ret = _spatialResampler->ResampleFrame(_captureFrame,&_resampledFrame);
			 }

			delete _spatialResampler;
	 }

	 int width =_resampledFrame.width();
	 int height = _resampledFrame.height();
	 char *pbuffer = (char*)malloc(width*height*3/2);
	 memcpy(pbuffer,_resampledFrame.buffer(kYPlane),width*height);
	 memcpy(pbuffer+width*height,_resampledFrame.buffer(kUPlane),width*height/4);
	 memcpy(pbuffer+width*height*5/4,_resampledFrame.buffer(kVPlane),width*height/4);*/
	const WebRtc_Word32 result = shared_data_->input_manager()->CreateVirtualDataPlayer(buffer,length,capabily,fileID);
	if (result != 0) {
		shared_data_->SetLastError(result);
		return -1;
	}
...
int ViEFileImpl::SendFileOnChannel(const int file_id, const int video_channel) {
  ...
  //m_fileidandchannelid_map.insert( std::make_pair(file_id,video_channel)); 
