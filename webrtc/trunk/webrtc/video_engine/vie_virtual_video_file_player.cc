...
#include "video_engine/vie_virtual_video_file_player.h"
...
ViEVirtualFilePlayer* ViEVirtualFilePlayer::CreateViEVirtualFilePlayer(
    int file_id,
    int engine_id,
    ...
    VoiceEngine* voe_ptr*/) {
  ViEVirtualFilePlayer* self = new ViEVirtualFilePlayer(file_id, engine_id);
...
ViEVirtualFilePlayer::ViEVirtualFilePlayer(int Id,
                             int engine_id)
...
ViEVirtualFilePlayer::~ViEVirtualFilePlayer() {
...
int ViEVirtualFilePlayer::Init(const char* file_nameUTF8,
                        const bool loop,
                        const FileFormats file_format/*,
  ...
  if (!feedback_cs_) {
    WEBRTC_TRACE(kTraceError, kTraceVideo, ViEId(engine_id_, id_),
                 "ViEVirtualFilePlayer::StartPlay() failed to allocate critsect");
    return -1;
  }
  ...
  if (!decode_event_) {
    WEBRTC_TRACE(kTraceError, kTraceVideo, ViEId(engine_id_, id_),
                 "ViEVirtualFilePlayer::StartPlay() failed to allocate event");
    return -1;
  }
  if (strlen(file_nameUTF8) > FileWrapper::kMaxFileNameSize) {
    WEBRTC_TRACE(kTraceError, kTraceVideo, ViEId(engine_id_, id_),
                 "ViEVirtualFilePlayer::StartPlay() Too long filename");
    return -1;
  }
...
*//*
  ...
  if (!file_player_) {
    WEBRTC_TRACE(kTraceError, kTraceVideo, ViEId(engine_id_, id_),
                 "ViEVirtualFilePlayer::StartPlay() failed to create file player");
    return -1;
  }
  if (file_player_->RegisterModuleFileCallback(this) == -1) {
    WEBRTC_TRACE(kTraceError, kTraceVideo, ViEId(engine_id_, id_),
                 "ViEVirtualFilePlayer::StartPlay() failed to "
...
int ViEVirtualFilePlayer::FrameCallbackChanged() {
    ...
    if (!play_back_started_) {
      ...
      if (decode_thread_->Start(thread_id)) {
        WEBRTC_TRACE(kTraceStateInfo, kTraceVideo, ViEId(engine_id_, id_),
                     "ViEVirtualFilePlayer::FrameCallbackChanged() Started file decode"
                     " thread %u", thread_id);
      } else {
        WEBRTC_TRACE(kTraceError, kTraceVideo, ViEId(engine_id_, id_),
                     "ViEVirtualFilePlayer::FrameCallbackChanged() Failed to start "
                     "file decode thread.");
      }
      ...
      if (file_player_->StartPlayingVideoFile(file_name_, false,
                                              !audio_stream_) != 0) {
        WEBRTC_TRACE(kTraceError, kTraceVideo, ViEId(engine_id_, id_),
                     "ViEVirtualFilePlayer::FrameCallbackChanged(), Failed to restart "
                     "the file player.");
      }
    }
  }
...
bool ViEVirtualFilePlayer::FilePlayDecodeThreadFunction(void* obj) {
  return static_cast<ViEVirtualFilePlayer*>(obj)->FilePlayDecodeProcess();
}

int ViEVirtualFilePlayer::FileDataDeProsess(){
 // file_player_->SetData(buffer,length);
	  ...
	  int size_uv = half_width * half_height;
	   ...
	   uint8_t* buffer_y = (uint8_t*)buffer;
	   uint8_t* buffer_u = (uint8_t*)buffer_y + size_y;
	   uint8_t* buffer_v =(uint8_t*) buffer_u + size_uv;
	  decoded_video_.CreateFrame(size_y, buffer_y,  size_uv, buffer_u, size_uv, buffer_v,  video_codec_info_.width, video_codec_info_.height,video_codec_info_.width, half_width, half_width);

  WebRtc_Word64 renderTimeMs = TickTime::MillisecondTimestamp();
  decoded_video_.set_render_time_ms(renderTimeMs);
DeliverFrame(&decoded_video_);

  return 0;
}

bool ViEVirtualFilePlayer::FilePlayDecodeProcess() {
...
/*
int ViEVirtualFilePlayer::StopPlay() {
  ...
  if (decode_thread_) {
    ...
    if (decode_thread_->Stop()) {
      ...
      assert(false && "ViEVirtualFilePlayer::StopPlay() Failed to stop decode thread");
      WEBRTC_TRACE(kTraceError, kTraceVideo, ViEId(engine_id_, id_),
                   "ViEVirtualFilePlayer::StartPlay() Failed to stop file decode "
                   "thread.");
    }
  }
  decode_thread_ = NULL;
...
*/
/*
int ViEVirtualFilePlayer::StopPlayAudio() {
  // Stop sending audio.
...
/*
int ViEVirtualFilePlayer::Read(void* buf, int len) {
...
/*
bool ViEVirtualFilePlayer::NeedsAudioFromFile(void* buf) {
...
*/
 /*
void ViEVirtualFilePlayer::PlayFileEnded(const WebRtc_Word32 id) {
...
/*
bool ViEVirtualFilePlayer::IsObserverRegistered() {
  CriticalSectionScoped lock(feedback_cs_);
  return observer_ != NULL;
}*/
/*
int ViEVirtualFilePlayer::RegisterObserver(ViEFileObserver* observer) {
  CriticalSectionScoped lock(feedback_cs_);
  if (observer_) {
    return -1;
  }
...
/*
int ViEVirtualFilePlayer::DeRegisterObserver() {
  CriticalSectionScoped lock(feedback_cs_);
  observer_ = NULL;
  return 0;
}*/
/*
int ViEVirtualFilePlayer::SendAudioOnChannel(const int audio_channel,
                                      bool mix_microphone,
  ...
  if (voe_file_interface_->StartPlayingFileAsMicrophone(audio_channel, this,
                                                       ...
                                                       volume_scaling) != 0) {
    WEBRTC_TRACE(kTraceError, kTraceVideo, ViEId(engine_id_, id_),
                 "ViEVirtualFilePlayer::SendAudioOnChannel() "
...
int ViEVirtualFilePlayer::StopSendAudioOnChannel(const int audio_channel) {
  int result = 0;
  if (!voe_file_interface_) {
    WEBRTC_TRACE(kTraceError, kTraceVideo, ViEId(engine_id_, id_),
                 "ViEVirtualFilePlayer::StopSendAudioOnChannel() - no VoE interface");
    return -1;
  }
  std::set<int>::iterator it = audio_channels_sending_.find(audio_channel);
  if (it == audio_channels_sending_.end()) {
    WEBRTC_TRACE(kTraceError, kTraceVideo, ViEId(engine_id_, id_),
                 "ViEVirtualFilePlayer::StopSendAudioOnChannel AudioChannel %d not "
  ...
  if (result != 0) {
    WEBRTC_TRACE(kTraceError, kTraceVideo, ViEId(engine_id_, id_),
                 "ViEVirtualFilePlayer::StopSendAudioOnChannel() "
...
*/
/*
int ViEVirtualFilePlayer::PlayAudioLocally(const int audio_channel,
                                    float volume_scaling) {
  if (!voe_file_interface_) {
    WEBRTC_TRACE(kTraceError, kTraceVideo, ViEId(engine_id_, id_),
                 "%s No VEFile interface.", __FUNCTION__);
...
int ViEVirtualFilePlayer::StopPlayAudioLocally(const int audio_channel) {
  if (!voe_file_interface_) {
    WEBRTC_TRACE(kTraceError, kTraceVideo, ViEId(engine_id_, id_),
                 "%s No VEFile interface.", __FUNCTION__);
    return -1;
...
int ViEVirtualFilePlayer::GetFileInformation(int engine_id,
