...
int ViECodecImpl::SetSendCodec(const int video_channel,
  ...
  WEBRTC_TRACE(kTraceInfo, kTraceVideo,
               ...
               "maxBr: %d, min_br: %d, frame_rate: %d"
               "numberOfSimulcastStreams: %u )", __FUNCTION__,
               video_codec.codecType, video_codec.plType, video_codec.width_used,
               video_codec.height_used, video_codec.startBitrate,
  ...
  __android_log_print(ANDROID_LOG_ERROR, "yyf","ViECodecImpl::SetSendCodec width = %d, height =%d\n", video_codec.width_used, video_codec.height_used);
  ...
  if (video_codec_internal.maxBitrate == 0) {
    ...
    video_codec_internal.maxBitrate = (video_codec_internal.width_used *
                                       video_codec_internal.height_used *
  ...
  if (frame_provider) {
    if (frame_provider->Id() >= kViECaptureIdBase &&
	  ...
	  if(vie_capture)
	  {
		  if(video_codec.codecType == kVideoCodecI420)
			  vie_capture->capture_module_->SetIsCapture4_3(1);
		  else
			  vie_capture->capture_module_->SetIsCapture4_3(0);
	  }
    }
  }
...
bool ViECodecImpl::CodecValid(const VideoCodec& video_codec) {
 ...
 } else if ((video_codec.codecType == kVideoCodecI420 &&
                  strncmp(video_codec.plName, "H264", 4) == 0) ||
			  (video_codec.codecType == kVideoCodecSVC &&
                  strncmp(video_codec.plName, "SVC", 3) == 0) ||
  ...
  if (video_codec.width_used > kViEMaxCodecWidth ||
      video_codec.height_used > kViEMaxCodecHeight) {
