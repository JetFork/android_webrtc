...
class ViERenderImpl
 ...
 public:
  ...
  virtual int AddRenderer(const int render_id, RawVideoType video_input_format,
   ...
   virtual int  Snapshot(const int window, const char *filename);
   
   virtual int SetRenderFrameDeliver(const int render_id,
                                  RenderFrameDeliver* renderDeliver);
								  
   virtual int  ConfigureRenderMode(const int render_id, const int scaledmode);

    virtual int GetLastRenderedFrameTime(const int render_id, const unsigned int window, int64_t &timestamp);
 protected:
