...
int VideoEngine::SetVideoJavaScClass(void* javaCmClass, void* javaCmDevInfoClass, void* javaCmCapabilityClass, void* javaRenderClass, void* javaRenderOpenGLClass)
{
#if defined(WEBRTC_ANDROID) && !defined(WEBRTC_CHROMIUM_BUILD)
  if (SetCaptureJavaScClass(javaCmClass, javaCmDevInfoClass, javaCmCapabilityClass) != 0) {
    WEBRTC_TRACE(kTraceError, kTraceVideo, g_vie_active_instance_counter,
                 "Could not set capture Android Class");
  }

  if (SetRenderJavaScClass(javaRenderClass, javaRenderOpenGLClass) != 0) {
    WEBRTC_TRACE(kTraceError, kTraceVideo, g_vie_active_instance_counter,
                 "Could not set capture Android Class");
  }
#endif
...
int VideoEngine::VideoSetCaptureAndroidVM(void* javaVM, void* javaContext) {
  WEBRTC_TRACE(kTraceApiCall, kTraceVideo, g_vie_active_instance_counter,
               "SetCaptureAndroidVM()");
...
int VideoEngine::VideoSetRenderAndroidVM(void* javaVM) {
  WEBRTC_TRACE(kTraceApiCall, kTraceVideo, g_vie_active_instance_counter,
               "SetRenderAndroidVM()");
...
#else
  WEBRTC_TRACE(kTraceError, kTraceVideo, g_vie_active_instance_counter,
               "WEBRTC_ANDROID not defined for VideoEngine::SetRenderAndroidVM");
  return -1;
#endif
}
