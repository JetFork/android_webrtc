...
//added by wuzhong
int ViEBaseImpl::ResumeSend(const int video_channel)
{
...
int ViEBaseImpl::PauseSend(const int video_channel) 
{
 ...
 int ViEBaseImpl::RegisterDecodedDataCallback(const int video_channel,DecodedDataCallback callback)
{
	...
	vie_channel->RegisterDecodedDataCallback(callback);
	 
	return 0;
}

  int ViEBaseImpl::DeregisterDecodedDataCallback(const int video_channel)
{
	...
	vie_channel->DeregisterDecodedDataCallback();
...
int ViEBaseImpl::GetVersion(char version[1024]) {
  ...
  version_stream << "VideoEngine 3.23.0" << std::endl;
