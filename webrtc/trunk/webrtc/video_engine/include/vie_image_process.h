...
class  ViEImageProcess {
 public:
  ...
  // Some cameras produce very noisy captured images, especially in low ight
  ...
  virtual int GetEnableDenoisingState(const int capture_id) = 0;//kmm add

  // This function enhances the colors on the decoded video stream, enabled by
  // default.
  virtual int EnableColorEnhancement(const int render_id,
                                     const bool enable) = 0;
  virtual int GetEnableColorEnhancementState(const int render_id) = 0;//kmm add

  virtual int EnableContrastEnhancement(const int render_id, const bool enable) = 0;//kmm add
  virtual int GetEnableContrastEnhancementState(const int render_id) = 0;//kmm add

  virtual int EnableSharpen(const int render_id, const bool enable) = 0;//kmm add
  virtual int GetEnableSharpenState(const int render_id) = 0;//kmm add
