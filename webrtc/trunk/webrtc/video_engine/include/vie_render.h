...
class WEBRTC_DLLEXPORT RenderFrameDeliver {
public:
    // This method is called when a new frame should be rendered.
    virtual int FrameDeliver(unsigned int number_of_streams) = 0;
    
protected:
    virtual ~RenderFrameDeliver() {}
};
...
class WEBRTC_DLLEXPORT ViERender {
 public:
  ...
  virtual int  Snapshot(const int render_id,
							const char *filename) = 0;
    
  virtual int SetRenderFrameDeliver(const int render_id,
                            RenderFrameDeliver* renderDeliver) = 0;
							
	virtual int  ConfigureRenderMode(const int render_id,
	   							const int scaledmode) = 0;
	virtual int GetLastRenderedFrameTime(const int render_id, const unsigned int window, int64_t &timestamp) = 0;
 protected:
  ViERender() {}
  virtual ~ViERender() {}
};
