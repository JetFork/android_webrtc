...
int  ViERenderImpl::Snapshot(const int render_id, const char *filename)
{
	WEBRTC_TRACE(kTraceApiCall, kTraceVideo,
		ViEId(shared_data_->instance_id(), render_id),
		"%s(window: %d)", __FUNCTION__, render_id);
	...
	renderer->Snapshot(filename);
	return 0;
}
int  ViERenderImpl::ConfigureRenderMode(const int render_id, const int scaledmode)
{
	WEBRTC_TRACE(kTraceApiCall, kTraceVideo,
		ViEId(shared_data_->instance_id(), render_id),
	...
	renderer->ConfigureRenderMode(scaledmode);
	return 0;
}
...
int ViERenderImpl::SetRenderFrameDeliver(const int render_id,
                                      RenderFrameDeliver* renderDeliver) {
    ...
    renderer->SetRenderFrameDeliver(renderDeliver);
    return 0;
...
int ViERenderImpl::GetLastRenderedFrameTime(const int render_id, const unsigned int window,
                                            int64_t &timestamp) {
    WEBRTC_TRACE(kTraceApiCall, kTraceVideo,
        ViEId(shared_data_->instance_id(), window),
        "%s(window: %d)", __FUNCTION__, window);
    ViERenderManagerScoped rs(*(shared_data_->render_manager()));
    ViERenderer* renderer = rs.Renderer(render_id);
    if (!renderer) {
        WEBRTC_TRACE(kTraceError, kTraceVideo,
    ...
    if (renderer->GetLastRenderedFrame(render_id, video_frame) != 0) {
        shared_data_->SetLastError(kViERenderUnknownError);
        return -1;
    }
    timestamp = video_frame.render_time_ms();
    return 0;
}
