  ...
  if (Started() && !EncoderActive() && !CaptureCapabilityFixed()) {
    ...
    if (best_width != 0 && best_height != 0 && best_frame_rate != 0) {
      if (best_width != capture_settings.width ||
        ...
        Stop();
       //deleted by wuzhong it can invoke camera setup function mulitipy times
...
WebRtc_Word32 ViECapturer::Start(const CaptureCapability& capture_capability) {
  ...
  if (EncoderActive()) {
    ...
    capability.width = codec_.width_used;
    capability.height = codec_.height_used;
    ...
    capability.interlaced = requested_capability_.interlaced;
  	...
  	__android_log_print(ANDROID_LOG_ERROR, "yyf","ViECapturer::Start width = %d, height = %d\n", capability.width, capability.height);
...
WebRtc_Word32 ViECapturer::SetCaptureFlashlight(
  const bool on) {
  WEBRTC_TRACE(kTraceStateInfo, kTraceVideo, ViEId(engine_id_, capture_id_), "%s",
               __FUNCTION__);
//  __android_log_print(ANDROID_LOG_DEBUG, "WEBRTC", "ViECapturer::SetCaptureFlashlight");
  return capture_module_->SetCaptureFlashlight(on);
}
...
//kmm add
bool ViECapturer::GetEnableDenoisingState()
{
	WEBRTC_TRACE(kTraceInfo, kTraceVideo, ViEId(engine_id_, capture_id_),
		"%s(capture_device_id: %d)", __FUNCTION__,
		capture_id_);
	CriticalSectionScoped cs(deliver_cs_.get());
	return denoising_enabled_;
}
//kmm end add
