...
namespace webrtc {
#ifdef GXH_TEST_H264
const int kMaxDecodeWaitTimeMs = 100;
#else
...
int ViEChannel::RegisterDecodedDataCallback(DecodedDataCallback callback)
{
	if(callback!=0)
	m_DecodedDataCallback =callback;
	else
		return -1;
	return  0;
}

int ViEChannel::DeregisterDecodedDataCallback()
{
  if(m_DecodedDataCallback!=NULL)
	  m_DecodedDataCallback =NULL;
  return 0;
}
  ...
  if (decoder_reset_) {
    ...
    if (codec_observer_) {
      ...
      if (vcm_.ReceiveCodec(&decoder) == VCM_OK) {
        ...
        // actually decoding.
        decoder.width_used = static_cast<uint16_t>(video_frame.width());
        decoder.height_used = static_cast<uint16_t>(video_frame.height());
...
//kmm del  if (color_enhancement_) {
	// VideoProcessingModule::ColorEnhancement(&video_frame);//kmm del:“—æ≠‘⁄‰÷»æ÷Æ«∞ø™∆Ù—’…´‘ˆ«ø
//  }
  ...
  //WEBRTC_TRACE(webrtc::kTraceStateInfo, webrtc::kTraceVideo, 0, "FrameToRender 111");
  DeliverFrame(&video_frame, no_of_csrcs, arr_ofCSRC);
  //WEBRTC_TRACE(webrtc::kTraceStateInfo, webrtc::kTraceVideo, 0, "FrameToRender 222"); 
  return 0;
}
...
WebRtc_Word32 ViEChannel::OnInitializeDecoder(
  ...
  vcm_.ResetDecoder();
  WEBRTC_TRACE(kTraceInfo, kTraceVideo, ViEId(engine_id_, channel_id_),
	  "%s: vcm_.ResetDecoder() is successful", __FUNCTION__);
  callback_cs_->Enter();
  decoder_reset_ = true;
  callback_cs_->Leave();

  WEBRTC_TRACE(kTraceInfo, kTraceVideo, ViEId(engine_id_, channel_id_),
	  "%s is successful", __FUNCTION__);
  return 0;
}
...
WebRtc_Word32 ViEChannel::ReceivedDecodedYUVData(const I420VideoFrame *_frame)
{

	if(!m_DecodedDataCallback)
		return -1;
	m_DecodedDataCallback((void*)_frame->buffer(kYPlane), (void*)_frame->buffer(kUPlane),(void*)_frame->buffer(kVPlane),_frame->width(),_frame->height());
	return 0;
}
}  // namespace webrtc
