...
bool VideoProcessorImpl::Init() {
  ...
  last_encoder_frame_width_ = config_.codec_settings->width_used;
  last_encoder_frame_height_ = config_.codec_settings->height_used;
  ...
  if (config_.verbose) {
    ...
    printf("    Width          : %d\n", config_.codec_settings->width_used);
    printf("    Height         : %d\n", config_.codec_settings->height_used);
  }
  initialized_ = true;
  return true;
}
...
bool VideoProcessorImpl::ProcessFrame(int frame_number) {
  ...
  if (frame_reader_->ReadFrame(source_buffer_)) {
    ...
    int size_y = config_.codec_settings->width_used * config_.codec_settings->height_used;
    int half_width = (config_.codec_settings->width_used + 1) / 2;
    int half_height = (config_.codec_settings->height_used + 1) / 2;
...
void VideoProcessorImpl::FrameDecoded(const I420VideoFrame& image) {
  ...
  if (image.width() !=  config_.codec_settings->width_used ||
      image.height() != config_.codec_settings->height_used) {
