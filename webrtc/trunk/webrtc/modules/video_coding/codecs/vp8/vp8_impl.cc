...
int VP8EncoderImpl::InitEncode(const VideoCodec* inst,
  ...
  if (inst->width_used < 1 || inst->height_used < 1) {
    return WEBRTC_VIDEO_CODEC_ERR_PARAMETER;
  }
  ...
  encoded_image_._size = CalcBufferSize(kI420, codec_.width_used, codec_.height_used);
  ...
  raw_ = vpx_img_wrap(NULL, IMG_FMT_I420, codec_.width_used, codec_.height_used,
                      1, NULL);
  // populate encoder configuration with default values
  if (vpx_codec_enc_config_default(vpx_codec_vp8_cx(), config_, 0)) {
    return WEBRTC_VIDEO_CODEC_ERROR;
  }
  config_->g_w = codec_.width_used;
  config_->g_h = codec_.height_used;
  ...
  if (codec_.width_used * codec_.height_used > 704 * 576 && number_of_cores > 1) {
...
int VP8EncoderImpl::Encode(const I420VideoFrame& input_image,
  ...
  if (input_image.width() != codec_.width_used ||
      input_image.height() != codec_.height_used) {
...
int VP8EncoderImpl::UpdateCodecFrameSize(const I420VideoFrame& input_image) {
  codec_.width_used = input_image.width();
  codec_.height_used = input_image.height();
  raw_->w = codec_.width_used;
  raw_->h = codec_.height_used;
  raw_->d_w = codec_.width_used;
  raw_->d_h = codec_.height_used;

  raw_->stride[VPX_PLANE_Y] = input_image.stride(kYPlane);
  raw_->stride[VPX_PLANE_U] = input_image.stride(kUPlane);
  raw_->stride[VPX_PLANE_V] = input_image.stride(kVPlane);
  vpx_img_set_rect(raw_, 0, 0, codec_.width_used, codec_.height_used);
