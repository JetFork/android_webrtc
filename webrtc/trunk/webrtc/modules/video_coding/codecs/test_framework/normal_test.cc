...
NormalTest::Encode()
{
    ...
    while (_lengthEncFrame == 0)
    {
        ...
        _inputVideoBuffer.set_width(_inst.width_used);
        _inputVideoBuffer.set_height(_inst.height_used);
        endtime = clock()/(double)CLOCKS_PER_SEC;

        _encodedVideoBuffer.SetHeight(_inst.height_used);
        _encodedVideoBuffer.SetWidth(_inst.width_used);
