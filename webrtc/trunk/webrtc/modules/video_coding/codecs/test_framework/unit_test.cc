...
UnitTest::Setup()
{
    ...
    _inst.width_used = _source->GetWidth();
    _inst.height_used = _source->GetHeight();
    _inst.codecSpecific.VP8.denoisingOn = true;

    // Get input frame.
    ASSERT_TRUE(fread(_refFrame, 1, _lengthSourceFrame, _sourceFile)
                           == _lengthSourceFrame);
    int size_y = _inst.width_used * _inst.height_used;
    int size_uv = ((_inst.width_used + 1) / 2)  * ((_inst.height_used + 1) / 2);
    _inputVideoBuffer.CreateFrame(size_y, _refFrame,
                                  size_uv, _refFrame + size_y,
                                  size_uv, _refFrame + size_y + size_uv,
                                  _inst.width_used, _inst.height_used,
                                  _inst.width_used,
                                  (_inst.width_used + 1) / 2, (_inst.width_used + 1) / 2);
    rewind(_sourceFile);
    ...
    _decodedVideoBuffer.CreateEmptyFrame(_inst.width_used, _inst.height_used, _inst.width_used,
                                         (_inst.width_used + 1) / 2,
                                         (_inst.width_used + 1) / 2);
    ...
    int i=0;
    _inputVideoBuffer.CreateEmptyFrame(_inst.width_used, _inst.height_used, _inst.width_used,
                                       (_inst.width_used + 1) / 2,
                                       (_inst.width_used + 1) / 2);
    while (frameLength == 0)
    {
...
UnitTest::Perform()
{
    ...
    // to verify that decoder is reset to a "fresh" state upon Reset().
    {
        ...
        // quarter size below.
        EXPECT_TRUE((_inst.width_used % 2 == 0) && (_inst.height_used % 2 == 0));

        VideoCodec tempInst;
        memcpy(&tempInst, &_inst, sizeof(VideoCodec));
        tempInst.width_used /= 2;
        tempInst.height_used /= 2;
        int tmpHalfWidth = (tempInst.width_used + 1) / 2;
        int tmpHalfHeight = (tempInst.height_used + 1) / 2;

        int tmpSizeY = tempInst.width_used * tempInst.height_used;
        ...
        tempInput.CreateFrame(tmpSizeY, _inputVideoBuffer.buffer(kYPlane),
                              ...
                              tempInst.width_used, tempInst.height_used,
                              tempInst.width_used, tmpHalfWidth, tmpHalfWidth);
