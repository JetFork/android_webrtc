...
VCMReceiver::VCMReceiver(VCMTiming* timing,
	...
	//__android_log_print(ANDROID_LOG_ERROR, "yyf","receiver create %d, %d, %d, %x", __LINE__, vcm_id_, receiver_id_, &jitter_buffer_);
...
int32_t VCMReceiver::InsertPacket(const VCMPacket& packet, uint16_t frame_width,
  ...
  assert(buffer);
  {
	...
	//__android_log_print(ANDROID_LOG_ERROR, "yyf","receiver insert %d, %d, %d, %x", __LINE__, vcm_id_, receiver_id_, &jitter_buffer_);
...
void VCMReceiver::ReceiveStatistics(uint32_t* bitrate,
  ...
  assert(bitrate);
  assert(framerate);
	//__android_log_print(ANDROID_LOG_ERROR, "yyf","receiver %d, %d, %d, %x", __LINE__, vcm_id_, receiver_id_, &jitter_buffer_);
