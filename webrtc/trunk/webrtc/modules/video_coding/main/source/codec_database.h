...
* @defgroup	API_Codec
*
* @brief	webRTC Codec接口，衔接API模块和codec模块
*
* API通过调用结构函数可以创建编(解)码器、初始化编(解)码器、调用编(解)码方法、关闭编(解)码器。
*/
...
class VCMCodecDataBase {
 public:
  ...
  VCMGenericDecoder*GetDecoderPrivate( uint8_t payload_type);
...
/**
* @ingroup	API_Codec
* @brief    创建编码器（CreateEncoder）
*
* @req		GVE-SRS-101
* @param[in]     const VideoCodecType type, 编码类型，VP8或H264等
* @returns   VCMGenericEncoder, 编码对象
*
* 在webrtc本身流程的基础上，再创建一个编码器类对象
*/
  // Create an internal encoder given a codec type.
  VCMGenericEncoder* CreateEncoder(const VideoCodecType type) const;
/**
* @ingroup	API_Codec
* @brief    关闭编码器（DeleteEncoder）
*
* @req		GVE-SRS-104
*
* 调用关闭SVC编码器用例
...
/**
* @ingroup	API_Codec
* @brief    创建解码器（CreateDecoder）
*
* @req		GGVE-SRS-105
* @param[in]     VideoCodecType type, 解码类型，VP8或H264等
* @returns   VCMGenericDecoder*, 解码对象
*
* 在webrtc本身流程的基础上，再创建一个解码器类对象
*/
