...
int VCMSessionInfo::MovePackets(PacketIterator start,
							    PacketIterator end)
...
{
#if 0
	WebRtc_UWord32 bytesToMove = 0;
	 WebRtc_UWord32 bytesToDelete = _packets[startIndex].sizeBytes;
	_packets[startIndex].Reset();
    ++_packetsNotDecodable;

	for (int j = startIndex + 1;j <= _highestPacketIndex; j++)
    {
        bytesToMove += _packets[j].sizeBytes; 
...
#ifndef GXH_TEST_H264	
	return_length += MovePackets(it, nalu_end);//(ptrStartOfLayer,packetIndex, endIndex);
#else
