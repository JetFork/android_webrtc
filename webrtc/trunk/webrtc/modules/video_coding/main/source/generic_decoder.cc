...
WebRtc_Word32 VCMDecodedFrameCallback::Decoded(I420VideoFrame& decodedImage)
{
    ...
    CriticalSectionScoped cs(_critSect);
	if (_receiveCallback != NULL)//kmm add
	{
		_receiveCallback->ReceivedDecodedYUVData(&decodedImage);
	}
    ...
    if (_receiveCallback != NULL)
    {
        ...
        //WEBRTC_TRACE(webrtc::kTraceStateInfo, webrtc::kTraceVideo, 0, "Decoded end 111");
		WebRtc_Word32 callbackReturn = _receiveCallback->FrameToRender(_frame);
		//WEBRTC_TRACE(webrtc::kTraceStateInfo, webrtc::kTraceVideo, 0, "Decoded end 222");

		//_receiveCallback->ReceivedDecodedYUVData(&_frame);

		//_receiveCallback->m_DecodedDataCallback(NULL,20,30);
