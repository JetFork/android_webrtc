...
_codecDataBase(id),
_receiveStatsTimer(3000, clock_),
_sendStatsTimer(3000, clock_),
_retransmissionTimer(10, clock_),
_keyRequestTimer(500, clock_)
//add by zj@2013-05-24
//,_network()
//add end
{
    assert(clock_);
	//__android_log_print(ANDROID_LOG_ERROR, "yyf","vcm create %d, %d", __LINE__, id);
#ifdef DEBUG_DECODER_BIT_STREAM
...
VideoCodingModule::Create(const WebRtc_Word32 id)
{
	...
	//__android_log_print(ANDROID_LOG_ERROR, "yyf","%d, %d", __LINE__, id);
...
/*        if (_receiveStatsCallback != NULL)
        {
            ...
            WebRtc_UWord32 frameRate;
            _receiver.ReceiveStatistics(&bitRate, &frameRate);
	//__android_log_print(ANDROID_LOG_ERROR, "yyf","stats: receive: fr: %d, br:%d", frameRate, bitRate);
    }
    ...
    if (_sendStatsTimer.TimeUntilProcess() == 0)
    {
        ...
        if (_sendStatsCallback != NULL)
        {
            ...
            WebRtc_UWord32 frameRate;
            {
                ...
                frameRate = static_cast<WebRtc_UWord32>(
                    _mediaOpt.SentFrameRate() + 0.5f);
		//__android_log_print(ANDROID_LOG_ERROR, "yyf","stats: send fr: %d, br:%d", frameRate, bitRate);
	
			//add by zj@2013-05-24
			//_network.frameRate = frameRate;
			//WEBRTC_TRACE(webrtc::kTraceStateInfo, webrtc::kTraceVideoCoding, 1,
			//	"Current actual bitrate is: %d",bitRate);
			//add end
                                         ...
                                         WebRtc_UWord32 maxPayloadSize)
{
    ...
    _mediaOpt.SetEncodingData(_sendCodecType,
                              ...
                              sendCodec->width_used,
                              sendCodec->height_used,
                              numLayers);
    ...
    if (ret < 0)
    {
        if (ret == VCM_ERROR_REQUEST_SLI)
        {
			...
			WEBRTC_TRACE(webrtc::kTraceError,
				webrtc::kTraceVideoCoding,
				VCMId(_id),
				"Failed to decode RequestSliceLossIndication");
        }
        else
        {
            WEBRTC_TRACE(webrtc::kTraceError,
                                    ...
                                    const WebRtcRTPHeader& rtpInfo)
{
	...
	//__android_log_print(ANDROID_LOG_ERROR, "yyf","insertpacket, %d, %d", __LINE__, _id);
