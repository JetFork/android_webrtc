...
QualityModesTest::Perform(const CmdArgs& args)
{
  ...
  for (int i = 0; i < NumberOfCodecs; i++)
  {
    ...
    if(strncmp(codec.plName,"VP8" , 5) == 0)
    {
      ...
      codec.width_used = (WebRtc_UWord16)_width;
      codec.height_used = (WebRtc_UWord16)_height;
                                                ...
                                                const WebRtc_UWord32 height)
{
  ...
  if (!retVal)
  {
    ...
    // now set new values:
    currentCodec.height_used = (WebRtc_UWord16)height;
    currentCodec.width_used = (WebRtc_UWord16)width;
