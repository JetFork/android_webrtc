...
CodecDataBaseTest::Perform(CmdArgs& args)
{
    ...
    sendCodec.width_used = 352;
    sendCodec.height_used = 288;
    VideoCodec currentSendCodec;
    _vcm->RegisterSendCodec(&sendCodec, 1, 1440);
    _vcm->SendCodec(&currentSendCodec);
    TEST(currentSendCodec.width_used == sendCodec.width_used &&
        currentSendCodec.height_used == sendCodec.height_used);
    sendCodec.width_used = 352/2;
    sendCodec.height_used = 288/2;
    _vcm->RegisterSendCodec(&sendCodec, 1, 1440);
    _vcm->SendCodec(&currentSendCodec);
    TEST(currentSendCodec.width_used == sendCodec.width_used &&
        currentSendCodec.height_used == sendCodec.height_used);
    ...
    receiveCodec.height_used = _height;
    receiveCodec.width_used = _width;
    ...
    _vcm->InitializeReceiver();
    sendCodec.width_used = _width;
    sendCodec.height_used = _height;
