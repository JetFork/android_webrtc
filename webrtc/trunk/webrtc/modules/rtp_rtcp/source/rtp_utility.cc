...
#define VCM_SVC_PAYLOAD_TYPE		37//124//120//VCM_I420_PAYLOAD_TYPE//36
#endif
...
#ifdef GXH_TEST_H264
  parsedPacket.header.payloadType    = PT == VCM_SPR_PAYLOAD_TYPE ? VCM_SVC_PAYLOAD_TYPE : PT;
#else
...
#ifndef GXH_TEST_H264
	else if(definedByProfile == RTP_SVC_HEADER_EXTENSION)
	{
		const WebRtc_UWord8* ptrRTPDataExtensionEnd = ptr + XLen;
		ParseSVCExtensionHeader(parsedPacket,
                                  ptrExtensionMap,
                                  ptrRTPDataExtensionEnd,
                                  ptr);
	}
...
#ifndef GXH_TEST_H264
void RTPHeaderParser::ParseSVCExtensionHeader(
    WebRtcRTPHeader& parsedPacket,
    const RtpHeaderExtensionMap* ptrExtensionMap,
    const WebRtc_UWord8* ptrRTPDataExtensionEnd,
    const WebRtc_UWord8* ptr) const {
	WebRtc_UWord16 globInfo = *((WebRtc_UWord16 *)&ptr[4]);
	parsedPacket.header.encIdx = (WebRtc_Word8)(globInfo & 0xF);
	parsedPacket.header.allocIdx = (WebRtc_Word8)((globInfo >> 4) & 0xF);
}
#endif
