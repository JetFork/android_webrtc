...
WebRtc_Word32 VideoFilePlayerImpl::GetVideoFromFile(I420VideoFrame& videoFrame)
{
    ...
    {
      int size_y = video_codec_info_.width_used * video_codec_info_.height_used;
      int half_width = (video_codec_info_.width_used + 1) / 2;
      int half_height = (video_codec_info_.height_used + 1) / 2;
      ...
      videoFrame.CreateFrame(size_y, buffer_y,
                             size_uv, buffer_u,
                             size_uv, buffer_v,
                             video_codec_info_.width_used, video_codec_info_.height_used,
                             video_codec_info_.width_used, half_width, half_width);
    }else
    {
...
int VideoFilePlayerImpl::SetData(char* buffer,unsigned int length)
{
	memcpy(_encodedData.payloadData,buffer,length);
	_encodedData.payloadSize = length;
	_encodedData.bufferSize	= length;
	...
	//timeToNextFrame = _accumulatedRenderTimeMs -(TickTime::Now() - _startTime).Milliseconds();
	return 0;
}
...
WebRtc_Word32 VideoFilePlayerImpl::SetUpVideoDecoder()
{
 ...
 file data.");
    ...
    // in a file.
    const WebRtc_UWord32 KReadBufferSize = 3 * video_codec_info_.width_used *
        video_codec_info_.height_used / 2;
    _encodedData.VerifyAndAllocate(KReadBufferSize);
    _encodedData.encodedHeight = video_codec_info_.height_used;
    _encodedData.encodedWidth = video_codec_info_.width_used;
