...
WebRtc_Word32 DeviceInfoAndroid::CreateCapabilityMap(
  ...
  if (VideoCaptureAndroid::AttachAndUseCaptureCapabilityObjects(
          env2,
          javaCmCaptureCapClass,
          attached2) != 0){
  	WEBRTC_TRACE(webrtc::kTraceError, webrtc::kTraceVideoCapture, _id,
...
//  jfieldID maxFpsField = env->GetFieldID(javaCapClassLocal, "maxFPS", "I");
  ...
  jfieldID widthField = env->GetFieldID(javaCmCaptureCapClass, "width", "I");
  jfieldID heigtField = env->GetFieldID(javaCmCaptureCapClass, "height", "I");
  jfieldID maxFpsField = env->GetFieldID(javaCmCaptureCapClass, "maxFPS", "I");
