...
namespace webrtc
{
void* globalVideoScCmClass = NULL;
void* globalVideoScCmDevInfoClass = NULL; 
void* globalVideoScjavaCmCapabilityClass = NULL;

WebRtc_Word32 SetCaptureJavaScClass(void* javaCmClass, void* javaCmDevInfoClass, void* javaCmCapabilityClass)
{
  globalVideoScCmClass = javaCmClass;
  globalVideoScCmDevInfoClass = javaCmDevInfoClass;         
  globalVideoScjavaCmCapabilityClass = javaCmCapabilityClass;
  __android_log_print(ANDROID_LOG_ERROR, "WEBRTC", "JNI:%s, globalVideoScCmClass:%p globalVideoScCmDevInfoClass:%p globalVideoScjavaCmCapabilityClass:%p", __FUNCTION__, globalVideoScCmClass, globalVideoScCmDevInfoClass, globalVideoScjavaCmCapabilityClass);
...
WebRtc_Word32 VideoCaptureAndroid::SetAndroidObjects(void* javaVM,
  ...
  __android_log_print(ANDROID_LOG_DEBUG, "yyf", "111111111111111111111111111JNI:%s g_jvm:%p g_javaContext:%p javaContext:%p", __FUNCTION__, g_jvm, g_javaContext, javaContext);
  ...
  if (javaVM) {
    ...
    g_javaCmClass = static_cast<jclass>(globalVideoScCmClass);
  ...
  __android_log_print(ANDROID_LOG_ERROR, "yyf", "registernatives ok, JNI:%s g_jvm:%p g_javaContext:%p javaContext:%p", __FUNCTION__, g_jvm, g_javaContext, javaContext);
    }
    else {
      WEBRTC_TRACE(webrtc::kTraceError, webrtc::kTraceVideoCapture, -1,
                   "%s: Failed to register native functions",
                   __FUNCTION__);
  __android_log_print(ANDROID_LOG_ERROR, "yyf", "registernatives fail, JNI:%s g_jvm:%p g_javaContext:%p javaContext:%p", __FUNCTION__, g_jvm, g_javaContext, javaContext);
      return -1;
    }

    g_javaCmDevInfoClass = static_cast<jclass>(globalVideoScCmDevInfoClass);
...
/////////////////////////////////////////////////////
    ...
    g_javaCmCapabilityClass = static_cast<jclass>(globalVideoScjavaCmCapabilityClass);
    
    // get java capture CaptureCapabilityAndroid
    /*jclass javaCmCapabilityClassLocal = env->FindClass(
        AndroidJavaCaptureCapabilityClass);
    if (!javaCmCapabilityClassLocal) {
      WEBRTC_TRACE(webrtc::kTraceError, webrtc::kTraceVideoCapture, -1,
                   "%s: could not find java class", __FUNCTION__);
      return -1;
    }
    ...
    // (to tell JNI that we are referencing it
    // after this function has returned)
    g_javaCmCapabilityClass = static_cast<jclass>
        (env->NewGlobalRef(javaCmCapabilityClassLocal));
    if (!g_javaCmCapabilityClass) {
      WEBRTC_TRACE(webrtc::kTraceError, webrtc::kTraceVideoCapture, -1,
                   "%s: InitVideoEngineJava(): could not create Java "
                   "CaptureCapabilityAndroid class reference",
                   __FUNCTION__);
      return -1;
    }
    // Delete local class ref, we only use the global ref
    env->DeleteLocalRef(javaCmCapabilityClassLocal);*/
///////////////////////////////////////////////////////
	...
	env->DeleteGlobalRef(g_javaCmCapabilityClass);
...
WebRtc_Word32 VideoCaptureAndroid::AttachAndUseCaptureCapabilityObjects(
    JNIEnv*& env,
    jclass& javaCmCaptureCapClass,
    bool& attached)
{
  ...
  javaCmCaptureCapClass = g_javaCmCapabilityClass;
...
void JNICALL VideoCaptureAndroid::ProvideCameraFrame(JNIEnv * env,
  ...
  WEBRTC_TRACE(webrtc::kTraceInfo, webrtc::kTraceVideoCapture,
    ...
    __android_log_print(ANDROID_LOG_ERROR, "yyf", "native ProvideCameraFrame %d, %lu", length, (long)context);
...
__android_log_print(ANDROID_LOG_ERROR, "yyf", "GetBestMatchedCapability, rawType:%d, codecType:%d", _frameInfo.rawType, _frameInfo.codecType);
...
WebRtc_Word32 VideoCaptureAndroid::SetCaptureFlashlight(
    bool on) {
  CriticalSectionScoped cs(&_apiCs);
  WEBRTC_TRACE(kTraceStateInfo, kTraceVideo, -1, "%s",
               __FUNCTION__);
//    __android_log_print(ANDROID_LOG_DEBUG, "WEBRTC", "VideoCaptureAndroid::SetCaptureFlashlight");
	if (!g_jvm)
	  return -1;
    ...
    jmethodID cid = env->GetMethodID(g_javaCmClass, "SetCaptureFlashlight",
                                     "(Z)V");
    if (cid == NULL) {
      WEBRTC_TRACE(webrtc::kTraceError, webrtc::kTraceVideoCapture, -1,
                   "%s: could not get java SetCaptureFlashlight ID",
                   __FUNCTION__);
      return -1;
    }
    env->CallVoidMethod(_javaCaptureObj, cid, on);
//	__android_log_print(ANDROID_LOG_DEBUG, "WEBRTC", "VideoCaptureAndroid::SetCaptureFlashlight");
    // Detach this thread if it was attached
    if (isAttached) {
      if (g_jvm->DetachCurrentThread() < 0) {
