...
namespace videocapturemodule
{
    ...
    int _strnicmp(char *str1, char *str2, unsigned maxlen)
    {
        //modified by wuzhong
        return 1;
    } 
...
__android_log_print(ANDROID_LOG_DEBUG, "WEBRTC", "3099999999 get bestformat total=%d", numberOfCapabilies); 
	for (WebRtc_Word32 tmp = 0; tmp < numberOfCapabilies; ++tmp) // Loop through all capabilities
    {
        MapItem* item = _captureCapabilities.Find(tmp);
        if (!item)
        ...
        {
             bestformatIndex = tmp;
__android_log_print(ANDROID_LOG_DEBUG, "WEBRTC", "3244444444 get bestformat index=%d", bestformatIndex); 
       }
	}
#endif
...
//kmm add
int DeviceInfoImpl::CaptureResolutionAdapt(int &width, int &height)
{
	//@Todo
	return 0;
}
