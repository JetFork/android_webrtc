...
void* globalAudioScClass = NULL;

WebRtc_Word32 SetAudioDeviceJavaScClass(void* javaScClass)
{
  globalAudioScClass = javaScClass;
  __android_log_print(ANDROID_LOG_DEBUG, "WEBRTC", "JNI:%s, globalAudioScClass:%p", __FUNCTION__, globalAudioScClass);
  return 0;
}
...
WebRtc_Word32 AudioDeviceAndroidJni::SetAndroidAudioDeviceObjects(
    ...
    void* env,
    void* context) {
  __android_log_print(ANDROID_LOG_DEBUG, "WEBRTC", "JNI:%s javaVM:%p env:%p context:%p globalAudioScClass:%p", __FUNCTION__, javaVM, env, context, globalAudioScClass);

      WEBRTC_TRACE(kTraceError, kTraceAudioDevice, -1,
                   "setAndroidAudioDeviceObjects 2222");
  ...
  if (env) {
    ...
    globalScClass = reinterpret_cast<jclass>(globalAudioScClass);
...
AudioDeviceAndroidJni::AudioDeviceAndroidJni(const WebRtc_Word32 id) :
            ...
            _recAudioSource(7), // 1 is AudioSource.MIC which is our default
            _AudioManagerMode(3), // 0 is MODE_NORMAL, which is our default, 2 is MODE_IN_CALL, 3 is MODE_IN_COMMUNICATION
            _AudioStreamType(3), // 0 is STREAM_VOICE_CALL,  which is our default, 3 is STREAM_MUSIC
            _InputScaler(0), // 0 is not scaler ,1 is *2, 4 is not scale , 5  is /2
...
WebRtc_Word32 AudioDeviceAndroidJni::SetRecordingDevice(WebRtc_UWord16 index)
{
	...
	WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id,
                     "SetRecordingDevice initialized %d",index);
    
    _InputScaler = index >>12;
    _AudioManagerMode = (index >>8)&0xf;
    _AudioStreamType = (index >>4)&0xf;
    _recAudioSource = (index)&0xf;
    
    WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id,
                     "SetRecordingDevice _InputScaler %d _AudioManagerMode %d _AudioStreamType %d _recAudioSource %d",_InputScaler,_AudioManagerMode,_AudioStreamType,_recAudioSource);
...
WebRtc_Word32 AudioDeviceAndroidJni::InitPlayout()
{
    ...
    // Call java sc object method
    jint res = env->CallIntMethod(_javaScObj, initPlaybackID, samplingFreq, _AudioStreamType);
    if (res < 0)
    {
        WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id,
...
WebRtc_Word32 AudioDeviceAndroidJni::InitSampleRate()
{
    ...
    {
        // call java sc object method
        res = env->CallIntMethod(_javaScObj, initPlaybackID, samplingFreq, _AudioStreamType);
        if (res < 0)
        {
            switch (samplingFreq)
            {
...
//                         "total delay is %d", msPlayDelay + _delayRecording);
            ...
            // inputscaler >0 means right shift, <0 means left shift
/*            if (_InputScaler != 0 )
            {
            	if(_InputScaler > 4)
            	{
            		for (unsigned int i =0;i<2 * samplesToRec;i++)
            		{
            			int32_t temp = _recBuffer[i];
            			temp =  temp << _InputScaler;
        					if (temp > 32767) {
            				temp = 32767;
        					}
        					else if (temp < -32768) {
