...
/*
  if (new_frame->render_time_ms() + KOldRenderTimestampMS < time_now) {
   ...
   WEBRTC_TRACE(webrtc::kTraceStateInfo, webrtc::kTraceVideo, 0, "AddFrame too old frame");
   	return -1;
  }
  if (new_frame->render_time_ms() > time_now + KFutureRenderTimestampMS) {
    WEBRTC_TRACE(kTraceWarning, kTraceVideoRenderer, -1,
                 "%s: frame too long into the future.", __FUNCTION__);
   WEBRTC_TRACE(webrtc::kTraceStateInfo, webrtc::kTraceVideo, 0, "AddFrame frame too long into the future");
    return -1;
  }
*/
  // Get an empty frame
  ...
  if (!frame_to_add) {
    if (empty_frames_.GetSize() + incoming_frames_.GetSize() >
      ...
      WEBRTC_TRACE(kTraceWarning, kTraceVideoRenderer,
                   ...
                   KMaxNumberOfFrames);
   WEBRTC_TRACE(webrtc::kTraceStateInfo, webrtc::kTraceVideo, 0, "AddFrame too many frames");
      return -1;
    }
