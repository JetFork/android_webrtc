...
#define RENDER_BUFFER_WIDTH 720
#define RENDER_BUFFER_HEIGHT 540
namespace webrtc {
...
D3D9Channel::D3D9Channel(LPDIRECT3DDEVICE9 pd3DDevice,
                                 ...
                                 Trace* trace,EventWrapper* UpdateScreenEvent) :
    _width(0),
    _height(0),
    ...
    _stopHeight(0)
{
	...
	_UpdateScreen_Event=UpdateScreenEvent;
    ...
    if (_width != videoFrame.width() || _height != videoFrame.height())
    {
        if (FrameSizeChange(videoFrame.width(), videoFrame.height(), 1) == -1)
        {
			 ...
			 WEBRTC_TRACE(kTraceError, kTraceVideo,  0,
                 "%s - FrameSizeChange is failed.", __FUNCTION__,
                 0);
            return -1;
...
int VideoRenderDirect3D9::InitDevice()
{
    ...
    if (!_fullScreen)
    {
        _winWidth = RENDER_BUFFER_WIDTH;//_originalHwndRect.right - _originalHwndRect.left;
        _winHeight = RENDER_BUFFER_HEIGHT;//_originalHwndRect.bottom - _originalHwndRect.top;
    ...
    else
    {
        ...
        _d3dpp.BackBufferHeight = RENDER_BUFFER_WIDTH;
        _d3dpp.BackBufferWidth = RENDER_BUFFER_HEIGHT;
...
WebRtc_Word32 VideoRenderDirect3D9::Init()
{
   ...
   //deleted by wuzhong 20140211   _screenUpdateEvent->StartTimer(true, 1000 / monitorFreq);
        ...
        while (it != _d3dZorder.rend())
        {
            ...
            if (ddIt != _d3dChannels.end())
            {
                ...
                if (channelObj)
                {
                    ...
                    UpdateVerticeBuffer(_pVB, 0, startWidth-_offsetrender, startHeight-_offsetrender,
                                                                 ...
                                                                 const float bottom)
{
    ...
    D3D9Channel* d3dChannel = new D3D9Channel(_pd3dDevice,
                                                      &_refD3DCritsect, _trace,_screenUpdateEvent);
