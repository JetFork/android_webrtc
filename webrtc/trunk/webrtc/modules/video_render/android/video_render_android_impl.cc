...
__android_log_print(ANDROID_LOG_DEBUG, "WEBRTC", "JNI:%s javavm:%p", __FUNCTION__, javaVM);
  return 0;
}
#endif
...
WebRtc_Word32 VideoRenderAndroid::StopRender() {
  WEBRTC_TRACE(kTraceStateInfo, kTraceVideoRenderer, _id, "%s:", __FUNCTION__);
  {
    CriticalSectionScoped cs(&_critSect);
    if (!_javaRenderThread)
    {
...
bool VideoRenderAndroid::JavaRenderThreadProcess()
{
 ...
 // __android_log_print(ANDROID_LOG_DEBUG, "WEBRTC", "JNI:%s g_jvm:%p _javaRenderJniEnv:%p", __FUNCTION__, g_jvm, _javaRenderJniEnv);
