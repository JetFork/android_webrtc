...
void* globalVideoRenderScClass = NULL;
void* globalVideoRenderOpenGLScClass = NULL;

WebRtc_Word32 SetRenderJavaScClass(void* javaRenderClass, void* javaRenderOpenGLClass)
{
  globalVideoRenderScClass = javaRenderClass;
  globalVideoRenderOpenGLScClass = javaRenderOpenGLClass;
  __android_log_print(ANDROID_LOG_ERROR, "WEBRTC", "JNI:%s, globalVideoRenderScClass:%p", __FUNCTION__, globalVideoRenderScClass);
  return 0;
}
...
WebRtc_Word32 AndroidSurfaceViewRenderer::Init() {
  ...
  _javaRenderClass = reinterpret_cast<jclass>(globalVideoRenderScClass);
...
WebRtc_Word32 AndroidSurfaceViewChannel::Init(
  ...
  jclass javaRenderClass = reinterpret_cast<jclass>(globalVideoRenderScClass);
  ...
  _isNeedCreateBufferCid =
      env->GetMethodID(javaRenderClass,
                       "IsNeedCreateBuffer",
                       "()I");
  if (_isNeedCreateBufferCid == NULL) {
    WEBRTC_TRACE(kTraceError,
                 kTraceVideoRenderer,
                 _id,
                 "%s: could not get IsNeedCreateBuffer ID",
                 __FUNCTION__);
    return -1; /* exception thrown */
  }
//_isNeedCreateBufferCid
...
void AndroidSurfaceViewChannel::DeliverFrame(JNIEnv* jniEnv) {
  ...
  _renderCritSect.Enter();
  jint isNeedCreate = 
        (jniEnv)->CallIntMethod(_javaRenderObj, _isNeedCreateBufferCid,
                                 NULL);
  if (_bitmapWidth != _bufferToRender.width() ||
      _bitmapHeight != _bufferToRender.height()||
isNeedCreate == 1) {
	__android_log_print(ANDROID_LOG_DEBUG, "kxw","need create buffer %d ...", isNeedCreate);
...
WebRtc_Word32 AndroidSurfaceViewChannel::GetScreenResolution(int *width, int *height)
{
...
void AndroidSurfaceViewChannel::ConfigureRenderMode(const int scaledmode)
{
